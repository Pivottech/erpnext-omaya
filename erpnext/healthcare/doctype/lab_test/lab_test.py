...
def create_test_from_template(lab_test):
	for t in lab_test.test_templates:
		template = frappe.get_doc('Lab Test Template', t.test_template)
...
def create_compounds(template, lab_test, is_group):
	...
	for normal_test_template in template.normal_test_templates:
		if(check_patient_conditions(lab_test, normal_test_template)):
			normal = lab_test.append('normal_test_items')
			if is_group:
...
def check_patient_conditions(lab_test, normal_test_template):
	...
	patient_dob = frappe.db.get_value("Patient", lab_test.patient, "dob") or getdate()
	days = date_diff(getdate(), patient_dob)
	age = flt(days / 365)
	if not normal_test_template.age_from: normal_test_template.age_from = age
	if not normal_test_template.age_till: normal_test_template.age_till = age
	age_condition = True if age >= flt(normal_test_template.age_from) and age <= flt(normal_test_template.age_till) else False
	gender_condition = True if (not normal_test_template.sex or normal_test_template.sex == lab_test.patient_sex) else False
	return bool(age_condition and gender_condition)
