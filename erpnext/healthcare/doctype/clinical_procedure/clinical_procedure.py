...
class ClinicalProcedure(Document):
	...
	def on_trash(self):
		if self.stock_entry:
			frappe.db.delete("Stock Entry", {"name": self.stock_entry})
	def on_submit(self):
		self.start_procedure()
		self.complete_procedure()
	...
	def complete_procedure(self):
		...
		if self.items:
			...
			if customer:
				...
				csi = frappe.db.get_value("Clinical Procedure Template", self.procedure_template, "create_sales_invoice")
				if csi:
					create_sales_invoice_for_healthcare(frappe.db.get_value("Clinical Procedure Template", self.procedure_template, "sales_invoice_type"), self.patient, self.company, self.warehouse)
...
@frappe.whitelist()
def get_procedure_consumables(procedure_template, patient):
	items = get_items('Clinical Procedure Item', procedure_template, 'Clinical Procedure Template')
	items = get_items_prices(items, patient)
	return items

def get_items_prices(items, patient):
	for item in items:
		item.rate = get_price_list_rate_for({
...
def make_stock_entry(doc):
	...
	frappe.msgprint(str(stock_entry))
	stock_entry.stock_entry_type = 'Material Issue'
	stock_entry.from_warehouse = doc.warehouse
	stock_entry.company = doc.company
	expense_account = frappe.db.get_value("Warehouse",doc.warehouse,"expense_account") or get_account(None, 'expense_account', 'Healthcare Settings', doc.company)
