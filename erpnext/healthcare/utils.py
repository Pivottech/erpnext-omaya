...
def get_clinical_procedures_to_invoice(patient, company):
	...
	for procedure in procedures:
		...
		if procedure.invoice_separately_as_consumables and procedure.consume_stock \
			...
			procedure_items = frappe.get_list("Clinical Procedure Item", filters = {"parent": procedure.name}, fields = ["*"])
			for p in procedure_items:
				if not p.invoiced and p.invoice_separately_as_consumables:
					clinical_procedures_to_invoice.append({
						'reference_type': 'Clinical Procedure Item',
						'reference_name': p.name,
						'service': p.item_code,
						'rate': get_price_list_rate_for({
							"price_list": "Standard Buying",
							"uom": p.uom,
							"transaction_date": procedure.start_date
						}, p.item_code),
						"qty": p.qty
					})
...
def set_invoiced(item, method, ref_invoice=None):
	...
	if item.reference_dt == 'Direct Medication Entry Detail':
		frappe.db.set_value(item.reference_dt, item.reference_dn, 'invoiced', invoiced)
		dme = frappe.db.get_value("Direct Medication Entry Detail", item.reference_dn, "parent")
		dme_obj = frappe.get_doc("Direct Medication Entry",dme)
		dme_obj.check_invoiced()

	elif item.reference_dt == 'Patient Appointment':
	...
	elif item.reference_dt == 'Lab Test Template Detail':
		frappe.db.set_value(item.reference_dt, item.reference_dn, 'invoiced', invoiced)
		lte = frappe.db.get_value("Lab Test Template Detail", item.reference_dn, "parent")
		lte_obj = frappe.get_doc("Lab Test", lte)
		lte_obj.check_invoiced()
	else:
		frappe.db.set_value(item.reference_dt, item.reference_dn, 'invoiced', invoiced)
