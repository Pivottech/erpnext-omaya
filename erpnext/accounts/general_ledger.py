...
def save_entries(gl_map, adv_adj, update_outstanding, from_repost=False):
	...
	save_max_apl_to_voucher(gl_map)

def make_entry(args, adv_adj, update_outstanding, from_repost=False):
	args = add_apl_to_args(args)
...
def save_max_apl_to_voucher(gl_map):
	max_apl = {"name" : "" , "level" : 0}
	for entry in gl_map:
		apl_name = frappe.db.get_value("Account" , entry.account , "apl")
		apl_num = frappe.db.get_value("Accounting Permission Level" , apl_name , "level_num")
		if flt(apl_num) > flt(max_apl["level"]) :
			max_apl["level"] = apl_num
			max_apl["name"] = apl_name
	if gl_map:
		voucher = frappe.get_doc(gl_map[0].voucher_type , gl_map[0].voucher_no)
		voucher.apl = max_apl["name"]
		voucher.db_update()
def add_apl_to_args(args):
	level = frappe.db.get_value("Account" , args.get('account') , 'apl')
	args.update({'apl' : level})
	return args
