	...
	refresh: function(frm) {
		...
		if (frappe.boot.active_domains.includes("Healthcare")) {
			...
			if (cint(frm.doc.docstatus==0) && cur_frm.page.current_view_name!=="pos" && !frm.doc.is_return) {
				...
				frm.add_custom_button(__('Healthcare Items'), function(){
					frappe.call({
						...
						callback: function(res){
							...
							frappe.call({
								method: "erpnext.healthcare.pivot_utils.get_appointments_to_invoice",
								args: {
									patient: frm.doc.patient,
									company: frm.doc.company 
								},
							...
							for (var i = 0 ; i < list.length; i++){
								var row = {}
								row['item'] = list[i]['service'];
								row['qty'] = list[i].hasOwnProperty('qty') ? list[i]['qty'] : 0 ;
								row['rate'] = 0;
								row['income_account'] = "";
