...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		...
		"item_code": ["item_code", "item_name", "brand", "description", "qty", "base_rate",
			"buying_rate", "base_amount", "buying_amount","practitioner_amount", "gross_profit", "gross_profit_percent"],
		...
		"brand": ["brand", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount",
			"gross_profit", "gross_profit_percent"],
		"item_group": ["item_group", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount", "practitioner_amount",
			"gross_profit", "gross_profit_percent"],
		"customer": ["customer", "customer_group", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount", "practitioner_amount",
			"gross_profit", "gross_profit_percent"],
		"customer_group": ["customer_group", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount", "practitioner_amount",
			"gross_profit", "gross_profit_percent"],
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		"practitioner_amount": _("Practitioner Amount") + ":Currency/currency:100",
...
class GrossProfitGenerator(object):
	...
	def process(self):
		...
		for row in self.si_list:
			...
			# get practitioner amount 
			p_amount_list = frappe.db.sql("select sum(amount) a from `tabPractitioner Invoice Item` where docstatus = 1 and sales_invoice_item_name = %(sales_invoice_item)s", values = {"sales_invoice_item": row.item_row}, as_dict = 1)
			row.practitioner_amount = flt(p_amount_list[0].a, self.currency_precision) if p_amount_list else 0
			...
			if self.filters.get("group_by") != "Invoice":
				for i, row in enumerate(self.grouped[key]):
					if i==0:
						...
						new_row.practitioner_amount += flt(row.practitioner_amount, self.currency_precision)
	...
	def set_average_rate(self, new_row):
		new_row.gross_profit = flt(new_row.base_amount - new_row.buying_amount - new_row.practitioner_amount, self.currency_precision)
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				`tabSales Invoice Item`.cost_center, `tabSales Invoice Item`.practitioner_amount
				{sales_person_cols}
			from
