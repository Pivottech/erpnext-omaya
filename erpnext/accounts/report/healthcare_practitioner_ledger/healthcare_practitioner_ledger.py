...
def get_result_as_list(data, filters):
	...
	for d in data:
		if not d.get('posting_date'):
			balance, balance_in_account_currency = 0, 0
		if d.get('voucher_type') == "Practitioner Invoice":
			patient = frappe.db.get_value(d.get('voucher_type'), d.get('voucher_no'), "patient")
			d.update({'patient': patient})
			d.update({'dossier_id': frappe.db.get_value(d.get('voucher_type'), d.get('voucher_no'), "dossier_id")})
		elif d.get('voucher_type') == "Journal Entry":
			patient = frappe.db.get_value(d.get('voucher_type'), d.get('voucher_no'), "user_remark")
			d.update({'patient': patient})
